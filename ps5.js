...
showRhymesButton.addEventListener('click', () => {
    let i = wordInput.value;
    outputDescription.innerHTML = "Words that rhyme with " + wordInput.value;
    wordOutput.textContent = "...loading";
    const url = getDatamuseRhymeUrl(i)
    
    datamuseRequest(url, (data) => {
        let sorted = groupBy(data, 'numSyllables');
        wordOutput.textContent = '';
        const l = data.length
        for (let sy in sorted) {
            let s = document.createElement('h3');
            s.innerHTML = "Syllables: " + sy;
            wordOutput.append(s);
            let u = document.createElement('ul');
            for(let y = 0; y < sorted[sy].length; y++) {
                let list = document.createElement('li');
                list.innerHTML = "<li class=word>" + sorted[sy][y]['word'] + "<style> .save {color: white; background-color: green; border-radius: 2px;} </style><button class=save>Save</button></li>"
                u.append(list);
            }
        wordOutput.append(u);
        }
...
showSynonymsButton.addEventListener('click', () => {
    ...
    wordOutput.textContent = "...loading";
    const url = getDatamuseSimilarToUrl(i);
    ...
    datamuseRequest(url, (data) => {
        ...
        for (let item in data) {
            let list = document.createElement('li');
            list.innerHTML = "<li class=word>" + data[item]['word'] + "<style> .save {color: white; background-color: green; border-radius: 2px;} </style><button class=save>Save</button></li>"
            u.append(list);
        }
        wordOutput.append(u);
        if (data.length == 0) {
...
wordOutput.addEventListener('click', event => {
    if (event.target.matches('.save')) {
        savedWordsArray.push(event.target.parentElement.childNodes[0].textContent)
